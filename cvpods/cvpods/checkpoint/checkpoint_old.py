...
class Checkpointer(object):
    ...
    def ema_model_save(self, name: str, tag_checkpoint: bool = True, **kwargs: dict):
        """
        Dump model and checkpointables to a file.
        ...
        data = {}
        data["model"] = self.ema_model.state_dict()
        for key, obj in self.checkpointables.items():
            data[key] = obj.state_dict()
        data.update(kwargs)
    ...
    def load_ema(self, path: str):
        ...
        checkpoint = self._load_file(path)
        self._load_model_ema(checkpoint)
        if self.resume:
            for key, obj in self.checkpointables.items():
                if key in checkpoint:
                    logger.info("Loading {} from {}".format(key, path))
    ...
    def resume_or_load_ema(self, path: str, *, resume: bool = True):
        """
        If `resume` is True, this method attempts to resume from the last
        checkpoint, if exists. Otherwise, load checkpoint from the given path.
        This is useful when restarting an interrupted training job.
    ...
    def _load_model_ema(self, checkpoint: Any):
        """
        Load weights from a checkpoint.
        ...
        Args:
            checkpoint (Any): checkpoint contains the weights.
        """
        checkpoint_state_dict = checkpoint.pop("ema")
