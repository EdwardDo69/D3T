...
from .paths_route import _PREDEFINED_SPLITS_M3FDVIS
...
CONVERT_CLASS = {"People": "person",
               "Car": "car",
               "Motorcycle": "motorcycle"}
@DATASETS.register()
class M3fdVis(BaseDataset):
    def __init__(self, cfg, dataset_name, transforms=[], is_train=True):
        super(M3fdVis, self).__init__(cfg, dataset_name, transforms, is_train)

        image_root, split = _PREDEFINED_SPLITS_M3FDVIS["voc"][self.name]
    ...
    def _get_metadata(self):
        # fmt: off
        thing_classes = ('person', 'car', 'motorcycle')
        meta = {
            "thing_classes": thing_classes,
            "evaluator_type": _PREDEFINED_SPLITS_M3FDVIS["evaluator_type"]["voc"],
    ...
    def _load_annotations(self):
        ...
        for fileid in fileids:
            ...
            jpeg_file = os.path.join(dirname, "Vis", fileid + ".png")
            if not os.path.exists(jpeg_file):
                jpeg_file = os.path.join(dirname, "Vis", fileid + ".jpg")
            ...
            for obj in tree.findall("object"):
                ...
                #### choose clase
                if cls in CONVERT_CLASS.keys():
                    cls = CONVERT_CLASS[cls]
                else:
                    continue
