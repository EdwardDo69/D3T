...
from .paths_route import _PREDEFINED_SPLITS_M3FDIR
...
@DATASETS.register()
class M3fdIr(BaseDataset):
    def __init__(self, cfg, dataset_name, transforms=[], is_train=True):
        super(M3fdIr, self).__init__(cfg, dataset_name, transforms, is_train)

        image_root, split = _PREDEFINED_SPLITS_M3FDIR["voc"][self.name]
    ...
    def _get_metadata(self):
        # fmt: off
        thing_classes = ('People', 'Car', 'Motorcycle')
        meta = {
            "thing_classes": thing_classes,
            "evaluator_type": _PREDEFINED_SPLITS_M3FDIR["evaluator_type"]["voc"],
    ...
    def _load_annotations(self):
        ...
        for fileid in fileids:
            ...
            jpeg_file = os.path.join(dirname, "Ir", fileid + ".png")
            if not os.path.exists(jpeg_file):
                jpeg_file = os.path.join(dirname, "Ir", fileid + ".jpg")
...
CLASS_NAMES = ('People', 'Car', 'Motorcycle')