...
from .paths_route import _PREDEFINED_SPLITS_FlIR_ALIGN_RGB
...
@DATASETS.register()
class FlirAlignRgb(BaseDataset):
    def __init__(self, cfg, dataset_name, transforms=[], is_train=True):
        super(FlirAlignRgb, self).__init__(cfg, dataset_name, transforms, is_train)

        image_root, split = _PREDEFINED_SPLITS_FlIR_ALIGN_RGB["voc"][self.name]
    ...
    def _get_metadata(self):
        # fmt: off
        thing_classes = ('person', 'car', 'bicycle')
        meta = {
            "thing_classes": thing_classes,
            "evaluator_type": _PREDEFINED_SPLITS_FlIR_ALIGN_RGB["evaluator_type"]["voc"],
    ...
    def _load_annotations(self):
        ...
        for fileid in fileids:
            ...
            #RGB fileid
            RGB_fileid = fileid[:11] + 'RGB'
            jpeg_file = os.path.join(dirname, "JPEGImages", RGB_fileid + ".jpeg")
            if not os.path.exists(jpeg_file):
                jpeg_file = os.path.join(dirname, "JPEGImages", RGB_fileid + ".jpg")
