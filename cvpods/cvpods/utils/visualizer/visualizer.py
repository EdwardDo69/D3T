...
_LABEL_INDEX = {"car": 0, "person": 1, "bicycle": 2}
...
class Visualizer:
    def __init__(self, img_rgb, scale=1.0, instance_mode=ColorMode.SEGMENTATION):
        """
        Args:
    ...
    def draw_instance_predictions(self, predictions):
        ...
        scores2 = predictions.scores if predictions.has("scores") else None
        classes = predictions.pred_classes if predictions.has("pred_classes") else None
        classes2 = predictions.pred_classes if predictions.has("pred_classes") else None
        ########################
        tboxes = boxes.tensor
        ...
        for score1, class1 in zip(scores, classes):
            if score1 < 0.5:
                # print(score1, class1)
                # del boxes[i]
                del scores2[i]
        ...
        i = 0
        tboxes = torch.Tensor(tboxes)

        boxes2 = Boxes(tboxes)

        ########################

        labels = _create_text_labels(classes2, scores2, self.thing_classes)
    ...
    def overlay_instances(  # noqa:C901
        ...
        if areas is not None:
            ...
            if class_color:
                assigned_colors = [assigned_colors[_LABEL_INDEX[labels[idx].split(' ')[0]]] for idx in sorted_idxs]
            else:
